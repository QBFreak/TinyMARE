...
FUNCTION(sql) {
    ...
    if (rc != SQLITE_OK) {
        strcpy(buff, "#-1 SQL statement error.");
        return;
    }

    int b = 0;  // Output buffer pointer
    int c = 0;  // Current column
    int cp = 0; // Column text pointer
    char buff_out[8192];
    const char *col_text;
    ...
    if (sqlite3_step(res) == SQLITE_ROW) {
        // First row, start filling the buffer
        // Reset the column pointers
        cp = 0;
        c = 0;
        for(b=0;b<8191;b++) {
            // Check and see if we're out of columns
            if (c == sqlite3_column_count(res)) {
                // Next row
                if (b > 0) {
                    // Remove the trailing column delimiter
                    buff_out[--b] = rdelim;
                } else {
                    buff_out[b] = rdelim;
                }
                ...
                if (sqlite3_step(res) != SQLITE_ROW) {
                    // No more rows, break out of the for loop
                ...
                if (cp == 0) {
                    col_text = sqlite3_column_text(res, c);
                }
